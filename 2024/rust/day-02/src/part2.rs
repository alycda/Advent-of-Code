...
#[tracing::instrument]
pub fn process_v2(input: &str) -> miette::Result<String, AocError> {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    // let (good, unknown): (Vec<_>, Vec<_>) = input.lines().enumerate().partition(|(idx, line)| {
    ...
    //     signs.iter().filter(|(sign, _diff)| *sign == '=').count() == 1
    //         && signs.iter().all(|(_sign, diff)| *diff < 4)
    // });
    ...
    let output = input
        ...
        // .filter(|arg0: &&(usize, &str)| duplicates(**arg0))
        .filter(|arg0: &(usize, &str)| duplicates(*arg0))
        // .map(|arg0: &(usize, &str)| signs(*arg0))
        .map(signs)
        .filter(|(idx, nums, line)| {
            let h = nums.iter().fold(HashMap::new(), |mut acc, (sign, _diff)| {
            ...
            let k = h.keys().collect::<Vec<_>>();

            // can't fix, duplicate(s) and too many sign changes
            if k.len() > 2 {
                return false;
            }

            if k.len() == 1
                // first or last can have a gap of 3+ but not in the middle
                && nums[1..nums.len() - 1]
                    .iter()
            ...
            // remaining cases will have 2 keys
            // true

            // // this is a char, not a number so I'm using min to get what I want, but it should be max
            // let direction = dbg!(k[0].min(k[1]));

            let dominant_direction = h.iter().max_by_key(|(_k, v)| *v).map(|(k, _v)| *k);
            dbg!(dominant_direction);
            let mut escape_hatch = false;
            match dominant_direction {
                Some('+') => {
                    // all values must be increasing, can skip ONE using the escape hatch
        ...
        //     let mut escape_hatch = false;
        //     let mut peekable = line.split_whitespace().peekable();
        //     let nums = line.split_whitespace().for_each(|c| {
        //         let c = c.parse::<i32>().unwrap();
        //         peekable.next();
        //         let next = peekable.peek().unwrap_or(&"0");
        //         let next = next.parse::<i32>().unwrap();
        ...
        // for (idx, n) in nums.enumerate() {
        //     let n = n.parse::<i32>().unwrap();
        //     // let next = nums.clone().nth(idx + 1).unwrap_or("0").parse::<i32>().unwrap();
        //     // let diff = (n - next).abs();
        //     // dbg!(n, next, diff);
        // }
        //     true
    ...
    Ok(output.to_string())
    // Ok((good.len() + count).to_string())
}

// filters out lines with 2 or more duplicate numbers, not a single duplicate
// 1 2 1 2 4 -> filtered (1,2)
// 1 4 2 3 4 -> not filtered (4)
fn duplicates((_idx, line): (usize, &str)) -> bool {
    let nums = line
        .split_whitespace()
        .map(|c| c.parse::<i32>().unwrap())
    ...
    nums.values().filter(|&v| *v > 1).count() < 2
}

fn signs((idx, line): (usize, &str)) -> (usize, Vec<(char, i32)>, &str) {
    let mut peekable = line.split_whitespace().peekable();
...
mod tests {
    ...
    //     assert_eq!(process_v2(input).unwrap(), expected);
    // }
