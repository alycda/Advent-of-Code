...
struct Grid(usize, usize, Vec<(Position, Direction)>);
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let mut state_costs: HashMap<(Position, Direction), (usize, Vec<Day16>)> = HashMap::new();
...
fn reconstruct_paths(
    ...
    state_costs: &HashMap<(Position, Direction), (usize, Vec<Day16>)>,
    visited: &mut HashSet<(Position, Direction)>,
    optimal_tiles: &mut HashSet<Position>
) {
